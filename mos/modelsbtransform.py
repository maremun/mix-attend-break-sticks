...
class MoShead(nn.Module):
    ...
    def __init__(self, ntoken, ninp, nhid, nhidlast, encoder, lockdrop, tie_weights=False, n_experts=10):
        ...
        #self.d = 1
        self.reduce = nn.Linear(nhidlast, n_experts - 1)  # self.d)
    ...
    def forward(self, output, dropoutl):
        ...
        reduced = self.reduce(output.view(-1, self.nhidlast))  # [seq_len * batch_size x(n_experts-1)]
        transform = torch.distributions.transforms.StickBreakingTransform()
        pis = transform(reduced)  # [seq_len * batch_size x n_experts)]
